...
if IsChinese:
    model_ckpts = {'bert': "bert-base-chinese",
                   'albert': "albert-base-v2",
                   'roberta': "/home/qiu/IdeaProjects/ditto2_iod_berter/chinese-roberta-wwm-ext",
...
class MultiTaskNet(nn.Module):
    def __init__(self, task_configs=[],
        ...
        if bert_path == None:
            if lm == 'bert':
                if IsChinese:
                    self.bert = AutoModelForMaskedLM.from_pretrained(model_ckpts[lm])
                else:
                ...
                if IsChinese:
                        import os
                        # path = os.path.join(model_path, 'chinese-roberta-wwm-ext')
        ...
        for config in task_configs:
            ...
            if IsChinese:
                self.module_dict['%s_fc' % name] = nn.Linear(21128, vocab_size)
            else:

...
class GlobalStructures(object):
    ...
    def findValuationsByTONC(self, to):
        plist = []
    ...
    def checkIfConflictListTO(self, to_new):
        key = self.generateKeyStableTosIndex(to_new)
        toKeys = self.stableTOsIndex[key]
        tos = []
        for toKey in toKeys:
        ...
        #tids_list = [e[0] for e in tids_arr]
        # to_new.t0 < to_new.t1 for to_new, check whether conflict
        '''
        i_t0, j_t1 = -1, -1
        for i, e in enumerate(tids_list):
            if to_new.t0 == e:
                i_t0 = i
            if to_new.t1 == e:
                j_t1 = i
    ...
    def checkTOIsConflict(self, to):
        '''
            True: has conflict
            False: does not have conflict
        '''

...
def main():
    ...
    parser.add_argument('-data_dir', '--data_dir', type=str, default='../../data/nba_')
    ...
    parser.add_argument('-checkpoint', '--checkpoint', type=str, default='distilbert-base-uncased')
    ...
    model_class, tokenizer_class, pretrained_weights = (ppb.DistilBertModel, ppb.DistilBertTokenizer, arg_dict['checkpoint'])
    ...
    batch_size = 2048
    if 'comm' in arg_dict['data_dir']:
        batch_size = 512
    for i in range(len(input_ids) // batch_size + 1):
        batch_start, batch_end = i * batch_size, min((i + 1) * batch_size, len(input_ids))
        if batch_end <= batch_start:
            break
        attention_mask_s = torch.tensor(attention_mask[batch_start: batch_end]).to(device)
        input_ids_s = input_ids[batch_start: batch_end].to(device)
        ...
        if i == 0:
            sentence_embeddings = sentence_embeddings_s.detach().cpu()
        else:
            sentence_embeddings = torch.cat((sentence_embeddings, sentence_embeddings_s.detach().cpu()), 0)
    ...
    attention_mask = np.where(np.array(list(context.values())) != 0, 1, 0)
    ccs = [[k, v] for k, v in context.items()]
    input_ids = torch.tensor(list([e[1] for e in ccs]))
    input_ids_keys = [e[0] for e in ccs]
    #input_ids = torch.tensor(list(context.values()))
    batch_size = 2048
    if 'comm' in arg_dict['data_dir']:
        batch_size = 512
    for i in range(len(input_ids) // batch_size + 1):
        ...
        attribute_embeddings_s = model_output['last_hidden_state'][:, 0, :]
        attribute_embeddings_s = dict(zip(input_ids_keys[batch_start: batch_end], attribute_embeddings_s.detach().cpu().numpy()))
        #attribute_embeddings = {key: torch.tensor(value) for key, value in attribute_embeddings.items()}
        for key, value in attribute_embeddings_s.items():
